...
from sqlalchemy import (
    Boolean, Column, DateTime, Numeric as SQLDecimal, Enum, Float,
...
class DepositSlipStatus(str, enum.Enum):
    """DRID Lifecycle Status"""
    INITIATED = "INITIATED"       # Customer created slip, awaiting branch visit
    RETRIEVED = "RETRIEVED"       # Teller retrieved the slip
    VERIFIED = "VERIFIED"         # Teller verified details
    PROCESSING = "PROCESSING"     # Transaction being processed
    COMPLETED = "COMPLETED"       # Transaction completed successfully
    EXPIRED = "EXPIRED"           # Time window passed (60 min default)
    CANCELLED = "CANCELLED"       # Customer or teller cancelled
...
class Receipt(Base):
    ...
    # Digital Signature fields (SBP Compliance)
    digital_signature = Column(Text, nullable=True)  # Base64 encoded RSA signature
    signature_hash = Column(String(64), nullable=True)  # SHA-256 hash of signed payload
    signature_timestamp = Column(DateTime, nullable=True)  # When receipt was signed
    signature_algorithm = Column(String(50), default="RSA-SHA256", nullable=True)
    is_signature_valid = Column(Boolean, nullable=True)  # Cached validation result
...
class DigitalDepositSlip(Base):
    """
    Digital Deposit Slip - Pre-Branch Initiation
    DRID (Digital Reference ID) system for customer-initiated deposits
    """
    __tablename__ = "digital_deposit_slips"

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)

    # DRID - Digital Reference ID (unique, time-bound)
    drid = Column(String(30), unique=True, nullable=False, index=True)

    # Status lifecycle
    status = Column(Enum(DepositSlipStatus), default=DepositSlipStatus.INITIATED, nullable=False, index=True)

    # Time constraints
    expires_at = Column(DateTime, nullable=False, index=True)  # Default 60 minutes from creation
    validity_minutes = Column(Integer, default=60, nullable=False)

    # Transaction details (entered by customer)
    transaction_type = Column(Enum(TransactionType), nullable=False)
    customer_cnic = Column(String(15), nullable=False, index=True)
    customer_account = Column(String(20), nullable=False)
    amount = Column(SQLDecimal(15, 2), nullable=False)
    ...
    depositor_cnic = Column(String(15), nullable=True)
    depositor_phone = Column(String(20), nullable=True)
    depositor_relationship = Column(String(100), nullable=True)  # Self, Family, Agent, etc.

    # Channel info (where slip was created)
    channel = Column(Enum(Channel), nullable=False)
    device_info = Column(JSONB, nullable=True)
    ip_address = Column(String(45), nullable=True)

    # Customer info (resolved from CNIC)
    ...
    retrieved_by = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True)
    verified_at = Column(DateTime, nullable=True)
    ...
    cancelled_by = Column(UUID(as_uuid=True), nullable=True)  # Can be customer or teller
    ...
    confirmation_channel = Column(Enum(NotificationChannel), nullable=True)
    confirmation_sent_at = Column(DateTime, nullable=True)

    # QR Code for branch retrieval
    qr_code_data = Column(Text, nullable=True)
    ...
    retriever = relationship("User", foreign_keys=[retrieved_by])
    ...
    # Indexes
    __table_args__ = (
        Index('ix_deposit_slips_status_expires', 'status', 'expires_at'),
        Index('ix_deposit_slips_customer_status', 'customer_cnic', 'status'),
    )
