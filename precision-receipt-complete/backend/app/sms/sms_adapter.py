# app/sms/sms_adapter.py
"""
SMS Adapter for Precision Receipt System
Maps SMS input to existing DRID flow services
"""
...
from app.sms.sms_messages import SMSMessages
...
class SessionState(str, Enum):
    ...
    # Cheque flow states (SMS uses manual entry, not images)
    ...
    # SMS manual entry states
    CHEQUE_MANUAL_CONFIRM = "CHEQUE_MANUAL_CONFIRM"
    CHEQUE_NUMBER = "CHEQUE_NUMBER"
    CHEQUE_DATE = "CHEQUE_DATE"
    CHEQUE_BANK = "CHEQUE_BANK"
    CHEQUE_PAYEE = "CHEQUE_PAYEE"
...
class SMSAdapter:
    """
    Adapter that connects SMS messages to existing DRID services
    ...
    def save_qr_code_image(self, qr_code_base64: str, drid: str) -> Optional[str]:
        ...
        try:
            ...
            # Generate public URL
            # Use PUBLIC_URL if set (should be ngrok URL for SMS), otherwise use localhost:9001
    ...
    async def _send_qr_code_to_customer(self, phone_number: str, drid: str, qr_url: str) -> bool:
        """
        Send QR code image to customer via SMS

        Args:
            phone_number: Customer's phone number (may have sms: prefix)
        ...
        try:
            ...
            logger.error(f"Error sending QR code via SMS: {e}", exc_info=True)
            return False
    ...
    async def process_message(
        ...
        Args:
            phone_number: SMS phone number (with or without sms: prefix)
    ...
    async def _handle_state(
        ...
        state_handlers = {
            ...
            SessionState.CHEQUE_MANUAL_CONFIRM: self._handle_cheque_manual_confirm,
            SessionState.CHEQUE_NUMBER: self._handle_cheque_number,
            SessionState.CHEQUE_DATE: self._handle_cheque_date,
            SessionState.CHEQUE_BANK: self._handle_cheque_bank,
            SessionState.CHEQUE_PAYEE: self._handle_cheque_payee,
    ...
    async def _handle_deposit_type(
        ...
        if message == '1':
            ...
            # Cheque Deposit - SMS uses manual entry (no images)
    ...
    async def _handle_active_slip_options(
        ...
        if message == '1':
            ...
            slip, error = DRIDService.cancel_deposit_slip(
                self.db, drid, "CUSTOMER", "Cancelled by customer via SMS"
            )
            if error:
    ...
    async def _handle_confirm_overwrite(
        ...
        if message == '1':
            ...
            if existing_drid:
                slip, error = DRIDService.cancel_deposit_slip(
                    self.db, existing_drid, "CUSTOMER", "Cancelled by customer to create new slip via SMS"
                )
                if error:
    ...
    async def _create_deposit_slip(self, session: UserSession) -> str:
        ...
        try:
            ...
            slip, error = DRIDService.create_deposit_slip(
                ...
                amount=session.data.get('amount'),
                currency="PKR",
                narration=f"Deposit initiated via SMS",
                depositor_name=session.data.get('depositor_name'),
                depositor_cnic=session.data.get('depositor_cnic'),
                depositor_phone=session.data.get('depositor_phone'),
                depositor_relationship=session.data.get('depositor_relationship', 'SELF'),
                channel="WHATSAPP",  # Using SMS channel
                device_info={"source": "sms", "phone": session.phone_number},
                ip_address=None,
                validity_minutes=60,
...
_Reply with the option number_"""
    ...
    # ============================================
    # CHEQUE MANUAL ENTRY HANDLERS (SMS - no images)
    # ============================================

    async def _handle_cheque_manual_confirm(
        self,
        session: UserSession,
        message: str,
        original_message: str,
        ...
        """Handle confirmation to enter cheque details manually"""
        if message.lower() in ['ok', 'yes', '1']:
            # Initialize cheque_data
            session.data['cheque_data'] = {}
            session.state = SessionState.CHEQUE_NUMBER
            return self.messages.CHEQUE_NUMBER_REQUEST
        else:
            # Cancel
            session.reset()
            return self.messages.CANCELLED

    async def _handle_cheque_number(
        self,
        session: UserSession,
        message: str,
        original_message: str,
        ...
        """Handle cheque number input"""
        import re
        cheque_num = original_message.strip()
        # Validate 6-10 digits
        if not re.match(r'^\d{6,10}$', cheque_num):
            return "Invalid cheque number. Please enter 6-10 digits:"

        if 'cheque_data' not in session.data:
            session.data['cheque_data'] = {}
        session.data['cheque_data']['cheque_number'] = cheque_num

        session.state = SessionState.CHEQUE_DATE
        return self.messages.CHEQUE_DATE_REQUEST

    async def _handle_cheque_date(
        self,
        session: UserSession,
        message: str,
        original_message: str,
        ...
        if re.match(r'^\d{4}-\d{2}-\d{2}$', date_str):
            ...
            return "Invalid date format. Please use YYYY-MM-DD (e.g., 2026-02-20):"

        session.data['cheque_data']['cheque_date'] = formatted_date
        session.state = SessionState.CHEQUE_BANK
        return self.messages.CHEQUE_BANK_REQUEST

    async def _handle_cheque_bank(
        self,
        session: UserSession,
        message: str,
        original_message: str,
        ...
        """Handle cheque bank input"""
        bank_name = original_message.strip()
        if len(bank_name) < 3:
            return "Please enter a valid bank name:"

        session.data['cheque_data']['cheque_bank'] = bank_name
        session.state = SessionState.CHEQUE_PAYEE
        return self.messages.CHEQUE_PAYEE_REQUEST

    async def _handle_cheque_payee(
        self,
        session: UserSession,
        message: str,
        original_message: str,
        ...
        """Handle cheque payee input"""
        payee_name = original_message.strip()
        if len(payee_name) < 2:
            return "Please enter a valid payee name:"

        session.data['cheque_data']['cheque_payee_name'] = payee_name

        # After manual entry, ask for clearing type
